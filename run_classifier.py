...
class QuoraProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    lines = self._read_tsv(os.path.join(data_dir, "train_big_100000.csv"))
    examples = []
    for (i, line) in enumerate(lines):
      if i == 0:
          continue
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    lines = self._read_tsv(os.path.join(data_dir, "dev_big_10000.csv"))
    examples = []
    labels = []
    for (i, line) in enumerate(lines):
      if i == 0:
...
def get_eval(pred_result, real_label, label_list):
    label_map = {}
    for (i,label) in enumerate(label_list):
        label_map[label] = i
    predictions = list(itertools.islice(pred_result, len(real_label)))
    pred = []
    real = []
    for i,prediction in enumerate(predictions):
        pred.append(prediction['values'])
        real.append(label_map[real_label[i]])
    print(classification_report(real, pred))

def get_test(pred_result,save_file,test_num=56370):
    predictions = list(itertools.islice(pred_result, test_num))
...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    #eval_examples  = processor.get_dev_examples_(FLAGS.data_dir)
    ...
    get_eval(pred_result, real_labels, label_list)
    #get_test(pred_result,'results.csv')
